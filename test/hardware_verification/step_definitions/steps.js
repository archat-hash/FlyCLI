...
Given('the flight controller is connected via {string} at {int} baud', async function (path, baud) {
    controller = new SerialFlightController(path, baud);
    useCase = new ExecuteCliUseCase(controller);
});

When('I execute the CLI command {string}', { timeout: 20000 }, async function (command) {
    try {
...
When('I execute the reboot command {string}', { timeout: 10000 }, async function (command) {
    try {
...
Then('the output should contain {string}', function (expected) {
    assert.ok(lastOutput.includes(expected), `Expected output to contain "${expected}", but got:\n${lastOutput}`);
});

Then('the output should match {string}', function (pattern) {
    const regex = new RegExp(pattern, 'i');
    assert.ok(regex.test(lastOutput), `Expected output to match pattern "${pattern}", but got:\n"${lastOutput}"`);
});

Then('the output should be longer than {int} characters', function (length) {
...
Then('the output should be valid metadata', function () {
...
Then('the connection should be closed by the firmware', function () {
    // success if we are here
    assert.ok(true);
});

Then('I should be notified to reflash via DFU', function () {
    console.log('\n[!] WARNING: Device ready for DFU recovery.');
    assert.ok(true);
});

Then('the device should enter DFU mode', function () {
    assert.ok(true);
});

Given('the controller is disconnected', async function () {
    if (controller) {
        await controller.disconnect();
        controller = null;
    }
...
Then('the output should not be empty', function () {
    assert.ok(lastOutput && lastOutput.trim().length > 0, 'Expected output to not be empty');
});

Then('the data should be returned without errors', function () {

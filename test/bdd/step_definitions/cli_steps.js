...
Given('the controller returns valid {string} and {string} output', () => {
  // No mock needed, real chip will return data
});

When('I execute the CLI command {string}', async (cmd) => {
  response = await useCase.execute(cmd);
});

When('I run the {string} command', async (cmdName) => {
  if (cmdName === 'health') {
...
Then('the controller should have executed {string} and {string} commands', () => {
